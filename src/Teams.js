...
//   useEffect(() => {
//     setTeams(teamsData); // Always fetch the latest teams data
//   }, []);
...
//     requestAnimationFrame(() => {
//       document.querySelectorAll(".team-card, .player-card").forEach((card) => {
//         observer.observe(card);
//       });
//     });
...
//                 <div className="w-24 h-24 flex-shrink-0">
//                   <img src={player.profilePhoto} alt={`${player.firstName} ${player.lastName}`} className="w-full h-full object-cover rounded-full" />
//                 </div>
//                 <div className="flex-1 mt-3 sm:mt-2 text-center">
...
//           <div className="grid grid-cols-2 gap-x-10 gap-y-10 sm:grid-cols-3 lg:grid-cols-5 justify-center max-w-screen-lg mx-0 px-10">
//             {teams.map((team, index) => (
//               <div key={index} className="team-card w-34 h-48 bg-white rounded-lg shadow-lg cursor-pointer transform hover:scale-105 transition-transform mx-auto sm:w-40 sm:h-56 lg:w-44 lg:h-60 flex flex-col pb-4" onClick={() => setSelectedTeam(team)}>
//                 <div className="flex-grow bg-blue-200 rounded-lg overflow-hidden">
